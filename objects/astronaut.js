class Astronaut {
  ...
  draw() {
    ...
    if (x > CANVAS_WIDTH - this.width) {
      x = CANVAS_WIDTH - this.width;
      resetX = true;
    }
    if (y > CANVAS_HEIGHT - this.height) {
    ...
    if (this.poisonous > 0) {
      ...
      if (this.foods.has("broccoli"))
        image(this.foodImgs["broccoli"], x + 5, y - 15, 20, 50);
      ...
      if (this.foods.has("lettuce"))
        image(this.foodImgs["lettuce"], x + 35, y - 6, 16, 20);
      if (this.foods.has("potato"))
        image(this.foodImgs["potato"], x + 45, y - 5, 20, 40);
